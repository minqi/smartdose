...
class SafetyNetCenter(object):
	SAFETY_NET_WINDOW = datetime.timedelta(days=7)
	SAFETY_NET_TIMEOUT = datetime.timedelta(hours=4)
	SAFETY_NET_THRESHOLD = .8
			...
			for patient_reminder in patient_reminders:
				if patient_reminder.prescription.safety_net_on:
					...
					if patient_reminder.ack == True:
						acked_dose_count += 1
			if dose_count != 0:
				adherence_percentage_for_patients_list.append([patient, float(acked_dose_count)/float(dose_count)])
		return adherence_percentage_for_patients_list

	def _schedule_safety_net_messages_from_adherence_percentage_list(self, adherence_percentage_by_patients_list, threshold):
		"""
		...
		for adherence_percentage_by_patient in adherence_percentage_by_patients_list:
			...
			for safety_net_member in safety_net_members:
				dictionary['patient_relationship'] = SafetyNetRelationship.objects.get(source_patient=patient, target_patient=safety_net_member).target_to_source_relationship
	...
	def schedule_safety_net_messages(self, window_start, window_finish, threshold, timeout):
		"""
		Schedules a message to a safety net member to notify about missed doses. Safety net member will be notified of the number of
		doses missed between window_start and window_finish. A  medication is considered not taken if it has gone
		unacknowledged for longer than timeout. threshold is 0 <= threshold <= 1 (e.g. .8) and represents the cutoff between adherence and non adherence
		"""
		if threshold < 0 or threshold > 1:
			raise Exception("0 <= threshold <= 1 must be true")
		adherence_percentage_by_patients_list = self._compute_adherence_percentage_by_patients(window_start, window_finish, datetime.datetime.now(), timeout)
		self._schedule_safety_net_messages_from_adherence_percentage_list(adherence_percentage_by_patients_list, threshold)
